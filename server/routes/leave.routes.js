...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `INSERT INTO employee_leave_balances (user_id, leave_type, number_of_days, remaining_days, year)
         VALUES 
           ($1, 'annual', 21, 21, $2),
           ($1, 'sick', 30, 30, $2),
           ($1, 'unpaid', 0, 0, $2)
         RETURNING leave_type as "leaveType", number_of_days as "totalDays"`,
        [userId, year]
      );
