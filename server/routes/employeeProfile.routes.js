...
//     // Get compensation if admin/hr
//     if (canViewAll || profile.userId === req.user.id) {
//       const compensation = await pool.query(
//         `SELECT 
//           base_salary as "baseSalary",
//           salary_type as "salaryType",
...
//       if (compensation.rows.length > 0) {
//         formattedProfile.compensation = compensation.rows[0];
//       }
//     }
...
//     formattedProfile.leaveBalances = leaveBalances.rows;
    
//     // Get recent attendance summary (last 30 days)
//     const attendanceSummary = await pool.query(
//       `SELECT 
//         COUNT(DISTINCT DATE(timestamp AT TIME ZONE 'Africa/Harare')) as "daysWorked",
//         COUNT(CASE WHEN type = 'sign-in' THEN 1 END) as "signIns",
//         COUNT(CASE WHEN type = 'sign-out' THEN 1 END) as "signOuts"
...
//     formattedProfile.recentAttendance = attendanceSummary.rows[0];
...
//       // Update compensation if baseSalary provided
...
// router.get('/stats/overview', authenticateToken, checkPermission('view_employees'), async (req, res) => {
//   try {
...
//       `SELECT 
//         AVG(EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date))) as "avgYears"
//        FROM employee_profiles
//        WHERE employment_status = 'active'`
//     );
...
//       averageTenure: parseFloat(avgTenure.rows[0].avgYears || 0).toFixed(1)
//     });
//   } catch (error) {
//     console.error('Get employee statistics error:', error);
//     res.status(500).json({ error: 'Internal server error' });
