...
//           api.get('/api/attendance/summary/month'),
//         ]);
...
const DashboardView = () => {
  ...
  // ✅ FIX: Fetch weekly + monthly summary stats from backend
  useEffect(() => {
    const fetchSummaries = async () => {
      if (!token) return;
      ...
      try {
        const [weekRes, monthRes] = await Promise.all([
          api.get('attendance/summary'),       // ✅ FIXED: No leading slash
          api.get('attendance/summary/month'), // ✅ FIXED: No leading slash
        ]);
